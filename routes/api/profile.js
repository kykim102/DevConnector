...
router.put(
	...
	async (req, res) => {
		...
		try {
			// Find matching profile using user id and experience id
			// and setting new values to all other values except for experience ID
...
router.put(
	...
	async (req, res) => {
		...
		const newExp = { school, degree, fieldofstudy, from };
		if (to) newExp.to = to;
		if (current) newExp.current = current;
		if (description) newExp.description = description;

		try {
			// Find matching profile using user id and education id
			// and setting new values to all other values except for education ID
			const profile = await Profile.findOneAndUpdate(
				{ user: req.user.id, 'education._id': req.params.exp_id },
				{
					$set: {
						'education.$': { _id: req.params.exp_id, ...newExp },
					},
				},
				{ new: true }
			);
